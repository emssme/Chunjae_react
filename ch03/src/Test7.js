//useReducer: 현재 상태변수(state) 객체와 행동(action) 객체를 인자로 받아서 새로은 상태를 반환하는 훅
//useState보다 더 복잡하고. 다양한 변수나 객체를 관리할 수 있도록 하는 훅
import React, {useReducer, useState} from 'react'

const Test7 = ({ step =1, min=0, max=10}) => {
    const [initValue, setInitValue] = useState(0); //useState의 초기화
    ...
    const reducer = (state, action) => {
        switch(action.type) {
            ...
            case "COUNTDOWN" :
                return state.count > action.min ? { count: state.React = action.step } : action.min ;
            case "RANDOM" :
                return {
